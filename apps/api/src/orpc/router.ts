import { os } from './base';
import {
    listUsers,
    getUserByUsername,
    createProfile,
    updateUser,
    overview,
    listSpots,
    updateSpotGeneralInfo,
    updateSpotLocation,
    searchSpotAddresses,
    resolveSpotAddress,
    mergeSpots,
    deleteSpot,
    listMedia,
    createMedia,
    updateMedia,
    deleteMedia,
    listClips,
    listAdminMaps,
    createAdminMap,
    updateAdminMap,
    uploadMapImage,
    deleteAdminMap,
} from './routers/admin';
import { createAdminCity, deleteAdminCity, listAdminCities, updateAdminCity } from './routers/adminCities';
import { listCities } from './routers/cities';
import { fetchMap, listMaps } from './routers/maps';
import {
    getById,
    listBySpot,
    getSpotMediasAround,
    list,
    getHashtagMediasAround,
    listClipsBySpot,
    uploadToSpot,
} from './routers/media';
import { me } from './routers/profiles';
import {
    createSpot,
    getSpot,
    getSpotOverview,
    getSpotsGeoJSON,
    listInBounds,
    listByTags,
    reverseGeocodeSpot,
    addClipToSpot,
    getVideoInfo,
} from './routers/spots';

export const router = os.router({
    spots: {
        create: createSpot,
        getSpot,
        getSpotOverview,
        getSpotsGeoJSON,
        listInBounds,
        listByTags,
        reverseGeocode: reverseGeocodeSpot,
        addClipToSpot,
        getVideoInformation: getVideoInfo,
    },
    maps: {
        fetch: fetchMap,
        list: listMaps,
    },
    cities: {
        list: listCities,
    },
    media: {
        getById,
        listBySpot,
        getSpotMediasAround,
        list,
        getHashtagMediasAround,
        listClipsBySpot,
        uploadToSpot,
    },
    profiles: {
        me,
    },
    admin: {
        overview,
        users: {
            list: listUsers,
            getByUsername: getUserByUsername,
            createProfile,
            update: updateUser,
        },
        spots: {
            list: listSpots,
            updateGeneralInfo: updateSpotGeneralInfo,
            updateLocation: updateSpotLocation,
            searchAddresses: searchSpotAddresses,
            resolveAddress: resolveSpotAddress,
            merge: mergeSpots,
            delete: deleteSpot,
        },
        media: {
            list: listMedia,
            create: createMedia,
            update: updateMedia,
            delete: deleteMedia,
        },
        clips: {
            list: listClips,
        },
        maps: {
            list: listAdminMaps,
            create: createAdminMap,
            update: updateAdminMap,
            uploadImage: uploadMapImage,
            delete: deleteAdminMap,
        },
        cities: {
            list: listAdminCities,
            create: createAdminCity,
            update: updateAdminCity,
            delete: deleteAdminCity,
        },
    },
});
